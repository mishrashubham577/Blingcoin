...
class blingcoinNetwork(object):
	""" Class voor blingcoin networking """
	...
	def startNetworking(self):
		...
		blingcoin.threader.startBackgroundThread(self.runServer)
	...
	def handleServerInput(self, data, ip):
		''' Handle blingcoin packets '''


		# reply to broadcast packets
		if hasPacketPrefix(data, packet_sync_request):
			# todo: do proper sync, not send everything
			transactions = blingcoin.db.doQuery("SELECT hash from transactions WHERE hash != 'd34db33f'", result='all') # everything except genesis transaction
			for transaction in transactions:
				transactionHash = transaction[0]

				# send transaction
				transactionJSON = blingcoin.transactions.getJSONForTransaction(transactionHash)
				blingcoin.network.sendToNode(ip, packet_payment_new + zlib.compress(transactionJSON) )
				
				# send current confirmation
				difficulty,addition = blingcoin.db.doQuery("select difficulty,addition from confirmations where transactionHash = ?", (transactionHash,), result='one')
				data = "%s,%s,%s" % (transactionHash,str(difficulty),addition)
				blingcoin.network.sendToNode(ip, packet_payment_confirmation + data)


		if hasPacketPrefix(data, packet_payment_new):
			blingcoin.transactions.addTransactionJSON( zlib.decompress(data[len(packet_payment_new):]) )

		if hasPacketPrefix(data, packet_payment_confirmation):
			blingcoin.transactions.addConfirmationCSV(data[len(packet_payment_confirmation):])
			

	def runServer(self):
		''' Runs blingcoin UDP server '''
