...
def getJSONForTransaction(transactionHash):
	...
	transaction = blingcoin.db.doQuery("SELECT hash,timestamp from transactions WHERE hash = ?", (transactionHash,), result='one')
	...
	inputs = blingcoin.db.doQuery("SELECT previousOutput, publicKey, timestamp, signature from transactions_inputs WHERE transactionHash = ?", (transactionHash,), result='all')
	...
	outputs = blingcoin.db.doQuery("SELECT amount, address, outputHash from transactions_outputs WHERE transactionHash = ?", (transactionHash,), result='all')
...
def addTransactionJSON(data):
	...
	for oldInput in transaction['inputs']:
		...
		# input must exist
		amount, address = blingcoin.db.doQuery("SELECT amount, address from transactions_outputs WHERE outputHash = ?", (outputHash,), result='one')
		...
		publicKey = blingcoin.wallets.decompressPublicKey(publicKey)

		signature = blingcoin.ecc.verify_signature(publicKey, message, signature)
			...
			if address == myAddress:
				...
				if myMoney >= transferAmount:
					...
					for oldOutput in usedOutputs:
						...
						signature = blingcoin.ecc.sign_message(int(privateKey), messageToSign)

						correct = blingcoin.ecc.verify_signature(blingcoin.wallets.decompressPublicKey(publicKey), messageToSign, signature)
						# assert correct == True
					...
					blingcoin.network.broadcastTransaction(transactionJSON)
...
def createTransactionConfirmation(transactionHash, timestamp):
	...
	while True:
		...
		solution = blingcoin.hasher(transactionValue + addition).hexdigest()
...
def addConfirmationCSV(data):
	...
	maxDifficulty = blingcoin.db.doQuery("select max(difficulty) from confirmations where transactionHash = ?", (transactionHash,), result='one')[0]
	
	if maxDifficulty >= difficulty:
		return
	
	timestamp = blingcoin.db.doQuery("select timestamp from transactions where hash = ?", (str(transactionHash),), result='one')[0]
	...
	if timestamp:
		...
		if solution.count('0') == difficulty:
			...
			blingcoin.db.doQuery('DELETE FROM confirmations WHERE transactionHash = ?', (transactionHash, ), result='none')
